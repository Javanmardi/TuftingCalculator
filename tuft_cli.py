...
#     gauge = parse_number(input("Enter gauge (inch, e.g. 1/8 or 0.125): "))
#     stitch = parse_number(input("Enter stitch rate (per dm): "))
#     pile = parse_number(input("Enter pile height (mm): "))
#     den = parse_number(input("Enter yarn denier (den): "))
#     pile_type = validate_pile_type(input("Enter pile type (Cut or Loop): ").strip())
#     return gauge, stitch, pile, den, pile_type
...
if __name__ == "__main__":
    ...
    if args.gauge and args.stitch and args.pile and args.den:
        # Command-line mode
        gauge = parse_number(args.gauge)
        stitch = parse_number(args.stitch)
        pile = parse_number(args.pile)
        ...
        # Interactive mode
        # gauge, stitch, pile, den, pile_type = prompt_interactive()
